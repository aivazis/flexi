...
# declaration
class Plexus(pyre.plexus, family='flexi.shells.plexus'):
    """
    ...
    def pyre_banner(self):
        ...
        """
        # the project header
        yield from textwrap.dedent(flexi.meta.banner).splitlines()
        # the doc string
        yield from self.pyre_showSummary(indent='')
        # the authors
        yield from textwrap.dedent(flexi.meta.authors).splitlines()
        # all done
        return
    ...
    def pyre_interactiveSessionContext(self, context=None):
        ...
        context['flexi'] = flexi  # my package
        # and chain up
    ...
    def pyre_mountApplicationFolders(self, pfs, prefix, **kwds):
        ...
        # if all goes well and we reach the intended folder without errors
            ...
            self._ux = flexi.ux.dispatcher(plexus=self, docroot=docroot, pfs=pfs)
