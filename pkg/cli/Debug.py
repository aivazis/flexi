...
# declaration
class Debug(flexi.shells.command, family='flexi.cli.debug'):
    """
    ...
    def nfs(self, plexus, **kwds):
        ...
        """
        # make a channel
        channel = journal.info("flexi.cli.nfs")
        # set up the indentation level
        indent = " " * 2

        # get the prefix
        prefix = "flexi" if self.root is None else self.root
        # and the name server
    ...
    def vfs(self, plexus, **kwds):
        ...
        prefix = flexi.primitives.path(
            "/flexi" if self.root is None else self.root)
        ...
        # make a channel
        channel = journal.info("flexi.cli.vfs")
        # sign in
